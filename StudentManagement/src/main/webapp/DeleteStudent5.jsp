...
<body>
    <div class="container">
        ...
        <% String idToDelete = request.getParameter("deleteId");
           if (idToDelete != null && !idToDelete.isEmpty()) {
               try {
                   ...
                   String sql = "DELETE FROM fifth WHERE IdNo = ?";
                   ...
                   conn.close();
                     if(rowsAffected>0)
                     {
                   			response.sendRedirect("DeleteStudent5.jsp?status=success");
                   			
                     }
                     else
                     {
                    	 response.sendRedirect("DeleteStudent5.jsp?status=error");
                     }
                   
               } catch (ClassNotFoundException | SQLException e) {
                   e.printStackTrace();
                   // Redirect to the same page with error message
                   response.sendRedirect("DeleteStudent5.jsp?status=error&message=" + e.getMessage());
               }
           }
        %>
        ...
        <form method="post" action="DeleteStudent5.jsp">
